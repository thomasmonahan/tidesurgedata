...
## 2D pressure and wind

The explorer also exposes an additive Dynamical spatial-field view. After selecting a site,
choose a UTC date/hour and field size, then click **Load 2D pressure and wind**. The plot shows
surface-pressure contours with thinned 10 m wind vectors. This uses `Dynamical.fetch_field()` and
`Dynamical.fetch_wind_field()` and does not change the existing point-Series or Recipe APIs.
