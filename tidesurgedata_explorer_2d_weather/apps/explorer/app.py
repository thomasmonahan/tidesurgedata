...
from data import (
    ...
    distance_km,
    utc_day_bounds,
    build_weather_field_source,
)
from map_view import make_map
from plots import line_figure, weather_field_figure
...
def cached_discovery(lat: float, lon: float, noaa_radius: float, usgs_radius: float) -> SiteSelection:
...
def main() -> None:
    ...
    st.caption("Click a US coastal location, discover nearby gauges, then retrieve and plot aligned data.")
    ...
    map_state = st_folium(fmap, height=500, use_container_width=True, returned_objects=["last_clicked"])
    ...
    if frame is not None:
        ...
        st.plotly_chart(line_figure(frame, discharge_cols, "River discharge"), use_container_width=True)
        st.plotly_chart(line_figure(frame, pressure_cols, "Surface pressure"), use_container_width=True)
        with st.expander("Aligned frame"):
            st.dataframe(frame, use_container_width=True)
        with st.expander("Provenance, licence and attribution", expanded=True):
            st.dataframe(st.session_state.training_provenance, use_container_width=True, hide_index=True)


    st.divider()
    st.subheader("2D pressure and wind field")
    st.caption(
        "Fetches a spatial Dynamical analysis field without changing the point-series Recipe queries."
    )
    field_left, field_mid, field_right = st.columns(3)
    with field_left:
        field_date = st.date_input("Field date", end_date, key="field_date")
    with field_mid:
        field_hour = st.slider("Field hour (UTC)", 0, 23, 12)
    with field_right:
        field_radius = st.slider("Field half-width (degrees)", 1.0, 10.0, 4.0, 0.5)
    wind_stride = st.slider("Wind-vector thinning", 1, 12, 4)

    if st.button("Load 2D pressure and wind"):
        try:
            field_time = pd.Timestamp(field_date, tz="UTC") + pd.Timedelta(hours=field_hour)
            source = build_weather_field_source(selection)
            bounds = dict(
                lat_min=max(-90.0, selection.lat - field_radius),
                lat_max=min(90.0, selection.lat + field_radius),
                lon_min=max(-180.0, selection.lon - field_radius),
                lon_max=min(180.0, selection.lon + field_radius),
            )
            with st.spinner("Retrieving Dynamical pressure and wind fields..."):
                pressure = source.fetch_field(field_time, **bounds)
                wind_u, wind_v = source.fetch_wind_field(field_time, **bounds)
            st.session_state.weather_pressure = pressure
            st.session_state.weather_wind_u = wind_u
            st.session_state.weather_wind_v = wind_v
        except Exception as exc:
            st.error(f"2D weather-field retrieval failed: {exc}")

    pressure = st.session_state.get("weather_pressure")
    wind_u = st.session_state.get("weather_wind_u")
    wind_v = st.session_state.get("weather_wind_v")
    if pressure is not None:
        st.plotly_chart(
            weather_field_figure(
                pressure,
                wind_u,
                wind_v,
                selected_lat=selection.lat,
                selected_lon=selection.lon,
                wind_stride=wind_stride,
            ),
            use_container_width=True,
        )
        st.caption(
            f"Requested Dynamical analysis time: {pressure.attrs.get('requested_time', '')} · "
            f"selected: {pressure.attrs.get('selected_time', '')}"
        )
