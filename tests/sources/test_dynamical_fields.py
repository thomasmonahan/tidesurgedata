...
# --- data path, offline (synthetic dataset in place of dynamical.org) ---------------------------


@pytest.fixture()
def offline_fields(monkeypatch: pytest.MonkeyPatch):
    """Serve a small synthetic analysis dataset with pressure and wind, without network access."""
    pytest.importorskip("xarray", reason="needs the 'met' extra")
    from .test_dynamical import FakeCollection, analysis_dataset

    ds = analysis_dataset()
    ds["wind_u_10m"] = ds["pressure_surface"] * 0 + 5.0
    ds["wind_v_10m"] = ds["pressure_surface"] * 0 - 2.0

    class Collection(FakeCollection):
        def __init__(self, dataset: str):
            super().__init__(dataset)
            self.extra_fields["cube:variables"].update(
                {"wind_u_10m": {"unit": "m s-1"}, "wind_v_10m": {"unit": "m s-1"}}
            )

    monkeypatch.setattr(Dynamical, "_collection", lambda self: Collection(self.dataset))
    monkeypatch.setattr(Dynamical, "_open_dataset", lambda self, collection=None: ds)
    return ds


def test_fetch_fields_returns_requested_subset(source: Dynamical, offline_fields) -> None:
    fields = source.fetch_fields(
        ["pressure_surface", "wind_u_10m", "wind_v_10m"],
        "2024-01-01T00:00Z",
        "2024-01-02T00:00Z",
        lat_min=51.0,
        lat_max=51.5,
    ...
    assert set(fields.data_vars) == {"pressure_surface", "wind_u_10m", "wind_v_10m"}
    assert fields.sizes["time"] == 24  # half-open: the sample at ``end`` is excluded
    assert sorted(fields.latitude.values) == [51.0, 51.5]
    assert sorted(fields.longitude.values) == [-3.5, -3.0]
    assert float(fields["wind_u_10m"].mean()) == pytest.approx(5.0)


def test_fetch_fields_unknown_variable(source: Dynamical, offline_fields) -> None:
...
def test_fetch_fields_no_data_in_range(source: Dynamical, offline_fields) -> None:
...
def test_fetch_fields_rejects_naive_times(source: Dynamical) -> None:
